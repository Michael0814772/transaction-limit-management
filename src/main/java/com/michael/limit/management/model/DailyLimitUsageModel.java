...
@NoArgsConstructor
@Entity
@Table(name = "apic_t_daily_limit_usage_details",
        uniqueConstraints = {
                @UniqueConstraint(columnNames = {
                        "request_id",
                        "transaction_request",
                        "tran_date"})})
public class DailyLimitUsageModel {

    @Id
    @SequenceGenerator(
            name = "apic_t_daily_limit_usage_details_sequence",
            sequenceName = "apic_t_daily_limit_usage_details_sequence",
            allocationSize = 1
    )
    @GeneratedValue(
            strategy = GenerationType.SEQUENCE,
            generator = "apic_t_daily_limit_usage_details_sequence"
    )
    @Column(name = "ID")
    private Long id;
    ...
    public void setLimitType(String limitType) {
        this.limitType = limitType.toUpperCase().trim();
    }
